...
def conv_forward_naive(x, w, b, conv_param):
  ...
  for n_i in range(0, N):
    for f_i in range(0, F):
      for h_i in range(0, h_):
        for w_i in range(0, w_):
...
def conv_backward_naive(dout, cache):
  ...
  # 窗口window 作为输入x， 反向传播时直接我们单独考虑滤波器得到的单独一个值
  # 而这个值就是由窗口与权重相乘得到的
  for i in range(0, N):
    for f in range(0, F):
...
def max_pool_forward_naive(x, pool_param):
  ...
  for n in range(N):
    for c in range(C):
      for h_n in range(H_new):
        for w_n in range(W_new):
          out[n, c, h_n, w_n] = np.max(x[n, c, h_n*s:h_n*s+pool_height, w_n*s:w_n*s+pool_width])
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
  ...
  for n in range(N):
    for c in range(C):
      for h_n in range(H_new):
        for w_n in range(W_new):
          window = x[n, c, h_n*s:h_n*s+pool_height, w_n*s:w_n*s+pool_width]
          h_axis, w_axis = np.unravel_index(np.argmax(window), window.shape)
          dx[n, c, h_n*s+h_axis, w_n*s+w_axis] = dout[n, c, h_n, w_n]
