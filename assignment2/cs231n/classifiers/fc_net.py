...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    # scores = np.dot(layer_1, self.params["W2"]) + self.params["b2"]
    ...
    dx_1, dw1, db1 = affine_relu_backward(dh1, cache_1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for index in range(self.num_layers):
      self.params["W"+str(index+1)] = np.random.normal(
        loc=0.0, scale=weight_scale, size=(layers_dims[index], layers_dims[index+1]))

      self.params["b"+str(index+1)] = np.zeros((layers_dims[index+1], ))
      if self.use_batchnorm and index <len(hidden_dims):
        self.params['gamma' + str(index+1)] = np.ones((layers_dims[index+1], ))
        self.params['beta' + str(index+1)] = np.zeros((layers_dims[index+1], ))    
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for index in range(self.num_layers-1):
      ...
      if self.use_batchnorm:
        ...
        h[index], cache1[index] = affine_forward(out[index], W, b)
        bn[index], cache2[index] = batchnorm_forward(h[index], gamma, beta, self.bn_params[index])
        out[index+1], cache3[index] = relu_forward(bn[index])
      else:
        out[index+1], cache3[index] = affine_relu_forward(out[index], W, b)

      if self.use_dropout:
        out[index+1], cache4[index] = dropout_forward(out[index+1], self.dropout_param)
    ...
    for i in range(t):
      if self.use_dropout:
        ddrop[t-i-1] = dropout_backward(dout[t-i], cache4[t-1-i])
      ...
      if self.use_batchnorm:
        ...
        dout[t-i-1], grads['W'+str(t-i)], grads['b'+str(t-i)] = affine_relu_backward(dout[t-i], cache3[t-1-i])
