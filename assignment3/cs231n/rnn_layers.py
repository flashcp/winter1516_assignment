...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  cache = []
  for i in range(T):
    h[:, i, :], cache_current = rnn_step_forward(x[:, i, :], curent_h, Wx, Wh, b)
    curent_h = h[:, i, :]
...
def rnn_backward(dh, cache):
  ...
  for i in range(T-1, -1, -1):
    dx[:, i, :], dprev_h_, dWx_, dWh_, db_= rnn_step_backward(dh[:, i, :] + dprev_h_, cache.pop())
    dh0 = dprev_h_
    dWx += dWx_
    dWh += dWh_
    db += db_
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  cache = []
  for i in range(T):
    h[:, i, :], next_c, cache_sub = lstm_step_forward(x[:, i, :], current_h, current_c, Wx, Wh, b)
    current_h = h[:, i, :]
    current_c = next_c
